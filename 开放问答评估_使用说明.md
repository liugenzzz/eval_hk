...
`pipeline.example.json` 是完整模板。每个模型只需写一次 `name/model_path`，预测文件自动约定为 `work_dir/<name>/<name>_<dataset>.xlsx`。外部预测用 `pred`，已评分明细用 `scored`，优先级是 `scored > pred > 约定路径`。

推理默认续传，分片保留在 `work_dir/<model>/_partial/`。提示词或其他指纹输入变化时会报错；确认要全量重跑时才加 `--overwrite`。完成后如需删除分片，加 `--clean-partial`。rubric sweep 串行执行，且不允许复用无法确认 rubric 版本的 `scored`。
...
| 裁判缓存(`cache_dir` 里的 `judge_cache_*.jsonl`) | 通常可保留；缓存键包含裁判提示词指纹，换 prompt 会自动失效重判 |
...
- `image_min_pixels` / `image_max_pixels` 是总像素面积阈值，不是宽高；必须成对设置为正整数且前者不大于后者。示例 `65536/589824` 与当前 LLaMAFactory `0.9.5.dev0` 训练配置一致。本项目先按 `llamafactory-0.9.5-qwen-static-v1` 预缩放，再交给 checkpoint processor；不配置时维持旧行为。降低 max 可减少视觉 token/显存但可能丢细节，提高 min 会放大小图。
...
这两个值也适用于 `pipeline.json` 的 `infer` 块。改值会改变推理 fingerprint/断点身份：统一入口用 `python -m eval_tool infer --config pipeline.json --overwrite` 或新的 `work_dir`；旧 `run_infer` 命令没有 `--overwrite` 开关，应把 `infer_config.json` 的 `overwrite` 设为 `true`，或使用新的 `out_dir`。

数据转换阶段与推理期规整相互独立：`eval_tool.convert_vqa_json` 直接读取并内嵌源图字节；仓库中 JPEG quality 90、最长边 1280 的离线变换实际在 `select_p123_data.py::encode_image`，不在 `aero_vqa_dataset .py`。若源图已被离线转换，这两个像素参数不会恢复原图。要与训练逐像素一致，还需保证源图字节、Pillow/Transformers 版本和 checkpoint processor 配置相同。
...
```json
{
  ...
  "judge": {
    ...
    "model": "qwen3.6-27b",
    "prompt_files": {
      "pointwise": "prompts/judge_equip_pointwise_v4.txt",
...
3. 换了提示词后指纹会自动变化；如果分数没更新，检查是否配了整份直读的 `scored`
