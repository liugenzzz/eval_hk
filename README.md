...
目标检测（指代定位 / 多框检测 / 物体识别 / 计数）这条通路的全部细节 —— 打分器注册表、画框的验收指标、评估集 `test.jsonl` 直读、D 组描述、派生评估集、四指纹、报表拆分 —— 见 **[docs/目标检测评估.md](docs/目标检测评估.md)**，配置样例见 `det.example.json`。

一句话概括这条通路的形状：**数据集声明自己的 `kind`，打分器按 kind 查注册表分派**。加一套数据（装备 / 书籍 / 目标检测）只写配置，不改 `run_eval`：

```json
"datasets": {
  "mcq": "aero_mcq",
  "book_tf": { "name": "book_tf", "kind": "choice", "params": { "choice_style": "judge" } },
  "ground":  { "name": "eval_set_v1", "kind": "grounding_single",
               "params": { "iou_gate": 0.5, "dev_threshold_pct": 5.0 } }
}
```

已实现的 kind：`choice` / `judge_text` / `grounding_single` / `grounding_multi` / `object_ident` / `short_answer` / `counting` / `inventory` / `exist_negative` / `describe` / `perturbation`。
