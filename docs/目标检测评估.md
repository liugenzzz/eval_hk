...
> 只想把它跑起来 → **[guides/目标检测评估_跑通流程.md](guides/目标检测评估_跑通流程.md)**。这份讲的是口径和为什么这么设计。
...
**② 这一步写进配置就不用手跑。** 顶层 `derive` 块声明每份派生集怎么造，
`eval_tool all` 会自动夹在两趟推理中间：
...
"derive": [
  ...
  {"dataset": "reverse_consistency", "mode": "reverse-consistency",
   ...
   "pools": {"region_identify": "<构建端>/prompts/region_identify/region_identify.txt"}},
  {"dataset": "question_perturbation", "mode": "question-perturbation",
   "variants": 3, "sample_ratio": 0.15,
   "pools": {"ground_appearance": "<构建端>/prompts/ground_attribute/ground_attribute.txt"}}
]
```

`source`（从哪份评估集派生）默认取第一个非派生数据集，`from`（用谁的预测）默认取唯一那个非基线模型 —— 派生集要的是**被测模型自己的**输出，拿基线的输出造出来的历史测不出这一轮 SFT 的链路。两个以上候选时直接报错，不猜。

下面这几条命令是同一件事的手动版，调试单个派生集时用：
