...
def _model_env_key(model_name: str) -> str:
    ...
    return f"{token.upper()}_MODEL_PATH"


def _apply_model_path_override(model_name: str, model_path: str | None):
    if not model_path:
        return

    os.environ[_model_env_key(model_name)] = model_path
    os.environ['LOCAL_VLM_MODEL_PATH'] = model_path

    builder = supported_VLM.get(model_name)
    if builder is None:
        return

    if hasattr(builder, 'keywords') and isinstance(builder.keywords, dict) and 'model_path' in builder.keywords:
        kwargs = dict(builder.keywords)
        kwargs['model_path'] = model_path
        supported_VLM[model_name] = partial(builder.func, *builder.args, **kwargs)
...
def _get_eval_sample_count(dataset):
    try:
        return len(dataset)
    except Exception:
    ...
    for row in rows:
        ...
        for idx, (primary_metric, primary_metric_value) in enumerate(metric_rows):
            report_rows.append({
                ...
                'eval_samples': row.get('eval_samples', '-') if idx == 0 else '',
...
def get_judge_kwargs(dataset_name, dataset_type, args):
    ...
    env_judge_key = os.environ.get('OPENAI_API_KEY_JUDGE', '')
    env_judge_base = os.environ.get('OPENAI_API_BASE_JUDGE', '')
    if env_judge_key:
        judge_kwargs['key'] = env_judge_key
    if env_judge_base:
        judge_kwargs['api_base'] = env_judge_base

    judge_base_url = args.judge_base_url or args.judge_api_base
    judge_key = args.judge_key or args.judge_api_key
    if judge_base_url:
        judge_base_url = judge_base_url.rstrip('/')
        if judge_base_url.endswith('/chat/completions'):
            judge_kwargs['api_base'] = judge_base_url
        else:
            judge_kwargs['api_base'] = f"{judge_base_url}/chat/completions"
    if judge_key:
        judge_kwargs['key'] = judge_key
...
"""
    ...
    parser.add_argument('--judge-model', type=str, default=None, help='Alias of --judge (legacy compatible)')
    ...
    parser.add_argument('--judge-api-base', type=str, default=None, help='Alias of --judge-base-url (legacy compatible)')
    parser.add_argument('--judge-api-key', type=str, default=None, help='Alias of --judge-key (legacy compatible)')
    ...
    parser.add_argument('--model-path', type=str, default=None,
                        help='Local model path override for model entries with `model_path` (legacy compatible).')
    ...
    if args.judge is None and args.judge_model is not None:
        args.judge = args.judge_model
    if args.judge_base_url is None and args.judge_api_base is not None:
        args.judge_base_url = args.judge_api_base
    if args.judge_key is None and args.judge_api_key is not None:
        args.judge_key = args.judge_api_key
...
def run_local_mode(args):
    ...
    if WORLD_SIZE > 1:
        ...
        logger.info(f'Initializing distributed backend: {dist_backend}')
    ...
    for _, model_name in enumerate(args.model):
        ...
        _apply_model_path_override(model_name, args.model_path)
        ...
        for _, dataset_name in enumerate(args.data):
            ...
            try:
                ...
                result_dataset_name = getattr(dataset, "dataset_name", dataset_name)
                if result_dataset_name != dataset_name:
                    result_file = get_pred_file_path(
                        str(pred_root), model_name, result_dataset_name, use_env_format=True)
                    if RANK == 0:
                        upsert_dataset_status(
                            run_dir=pred_root,
                            model_name=model_name,
                ...
                eval_samples = _get_eval_sample_count(dataset)
